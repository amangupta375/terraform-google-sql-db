...
| import\_existing\_instance | Set to true to import an existing instance instead of creating a new one. If true, 'import\_instance\_id' must be provided and resource variables (name, project, region, settings, etc.) MUST match the existing instance. | `bool` | `false` | no |
| import\_instance\_id | The ID of the existing Cloud SQL instance to import when import\_existing\_instance is true. Format: 'project/region/name' or 'project/name'. | `string` | `null` | no |
